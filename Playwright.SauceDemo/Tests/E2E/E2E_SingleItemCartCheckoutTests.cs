...
namespace Playwright.SauceDemo.Tests.E2E
{
   internal class E2E_SingleItemCartCheckoutTests : BaseTest
   {
      ...
      [Category("E2E")]
      [TestCaseSource(typeof(E2EProvider), nameof(E2EProvider.GetSingleItem))]
      public async Task E2E_SingleItemCartCheckoutFlow(E2ETestCase testData)
      {
         ...
         foreach (var product in products!)
         {
            ReportManager.Log(ReportInfo, $"Verifying that the added items to cart are still present");
      ...
      {
         public static IEnumerable<E2EData> GetData() => E2EProvider.GetSingleItem().Select(tc => tc.Data);
      }
   }
}
